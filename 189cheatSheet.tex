...
Given a set of points and classes $\{x_i,y_i\}_{i=1}^n$, test features $x_j$ and branch on the feature which ``best'' separates the data. Repeat on split data until good classification is reached.\\

{\bf Splitting Heurisitic}:\\
Maximize Information Gain: \(\max_j \ \operatorname{infogain}(D|X_j)\) \[ \max_j \ \ \operatorname{H}(D) \ - \sum_{x_j \in X_j} P(X_j = x_j) \cdot \operatorname{H}(D | X_j = x_j) \]
...
% Minimize Misclassification Impurity:

{\bf Regression}: Minimize variance
...
DT's a prone to label noise. Building the correct tree is NP hard.\\
